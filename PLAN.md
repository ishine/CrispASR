...
- **Open: performance pass.** Current AR step is ~137 ms/frame on M1 Metal — 1.7× slower than real-time at 12.5 fps. Optimizations explored so far (all gated, default OFF): `QWEN3_TTS_O15=1` (cp graph reuse, e2e PASS), `QWEN3_TTS_FUSED_QKV=1` (talker fused QKV, byte-identical), `QWEN3_TTS_LK_BUCKET=1` (talker Lk bucketing, byte-identical, **net loss on M1 Metal Q8_0 across three load conditions** — see LEARNINGS "Qwen3-TTS Lk bucketing — net loss on M1 Metal"). Still open: F16→Q8_0 KV cache, Q4_K talker fused QKV, and the bigger lift — fusing the 15 sequential code_predictor steps into a single graph (requires on-device top-k sampling).
