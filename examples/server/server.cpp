...
void whisper_print_usage(int /*argc*/, char** argv, const whisper_params& params, const server_params& sparams) {
    ...
    fprintf(stderr,
            "  -nlp,      --no-language-probabilities [%-7s] exclude language probabilities from verbose_json output\n",
...
bool whisper_params_parse(int argc, char** argv, whisper_params& params, server_params& sparams) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (arg == "-nlp" || arg == "--no-language-probabilities") {
            params.no_language_probabilities = true;
        }
...
std::string generate_temp_filename(const std::string& path, const std::string& prefix, const std::string& extension) {
    ...
    std::stringstream ss;
    ss << path << static_cast<char>(std::filesystem::path::preferred_separator) << prefix << "-"
       << std::put_time(std::localtime(&now_time_t), "%Y%m%d-%H%M%S") << "-" << dist(rng) << extension;
...
bool convert_to_wav(const std::string& temp_filename, std::string& error_resp) {
    ...
    cmd_stream << "ffmpeg -i \"" << temp_filename << "\" -y -ar 16000 -ac 1 -c:a pcm_s16le \""
    ...
    for (int i = s0; i < n_segments; i++) {
        ...
        if (params.print_colors) {
            for (int j = 0; j < whisper_full_n_tokens(ctx, i); ++j) {
                ...
                const int col =
                    std::max(0, std::min((int)k_colors.size() - 1, (int)(std::pow(p, 3) * float(k_colors.size()))));
...
std::string output_str(struct whisper_context* ctx, const whisper_params& params,
...
int main(int argc, char** argv) {
    ...
    <body>
        ...
        <h2>)" + sparams.request_path + sparams.inference_path +
                                        R"(</h2>
        <pre>
    curl 127.0.0.1:)" + std::to_string(sparams.port) +
    ...
    -F model="&lt;path-to-model-file&gt;"
        ...
        <div>
            <h2>Try it out</h2>
            <form action=")" + sparams.request_path +
                                        sparams.inference_path + R"(" method="POST" enctype="multipart/form-data">
    ...
    auto console_ctrl_handler =
        +[](DWORD ctrl_type) -> BOOL { return (ctrl_type == CTRL_C_EVENT) ? (signal_handler(SIGINT), true) : false; };
