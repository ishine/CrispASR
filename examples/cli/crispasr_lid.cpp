...
bool detect_with_whisper_tiny(const float* samples, int n_samples, const whisper_params& p, crispasr_lid_result& out) {
    ...
    const bool cache_miss = (c.ctx == nullptr) || (c.model_path != model_path) || (c.use_gpu != p.use_gpu) ||
                            (c.gpu_device != p.gpu_device) || (c.flash_attn != p.flash_attn);
    ...
    const int lang_id = whisper_lang_auto_detect(ctx, /*offset_ms=*/0, p.n_threads, probs.data());
    ...
    if (!p.no_prints) {
        fprintf(stderr, "crispasr[lid]: detected '%s' (p=%.3f) via whisper-tiny\n", out.lang_code.c_str(),
                out.confidence);
    }
...
bool detect_with_silero(const float* samples, int n_samples, const whisper_params& p, crispasr_lid_result& out) {
    ...
    if (model_path.empty() || model_path == "auto") {
        model_path = crispasr_cache::ensure_cached_file(
            ...
            "https://huggingface.co/cstr/silero-lid-lang95-GGUF/resolve/main/silero-lid-lang95-f32.gguf", p.no_prints,
    ...
    if (model_path.size() >= 5 && model_path.compare(model_path.size() - 5, 5, ".gguf") == 0) {
        ...
        if (lid) {
            ...
            if (lang && *lang) {
                ...
                std::string code = (comma != std::string::npos) ? full.substr(0, comma) : full;
                // Trim whitespace
